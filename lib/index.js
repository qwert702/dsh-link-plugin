...
export async function apply(ctx, config) {
  ...
  function makeClient(cfg) {
    ...
    c.on('ready', () => {
      console.error('[dsh-link] READY')
      // 配对成功后清空配置文件里的配对码(一次性),后续用持久化 token 重连
      if (cfg.pairingCode) {
        writeConfigFile({ pairingCode: '' }).catch(() => {})
        cfg.pairingCode = ''
        merged = { ...merged, pairingCode: '' }
      }
    })
  ...
  async function rebuild() {
    ...
    // 文件的空值不覆盖 entry(组合配置兜底)
    const nonEmpty = Object.fromEntries(Object.entries(file).filter(([, v]) => v !== '' && v !== undefined))
    merged = { ...entry, ...nonEmpty }
  ...
  // 首次启动:若配置文件不存在,把 entry 的默认值(serverUrl 等)持久化,设置页可直接看到
  try {
    await readFile(CONFIG_PATH, 'utf8')
  } catch {
    const defaults = Object.fromEntries(
      Object.entries({ serverUrl: entry.serverUrl, profile: entry.profile, proxy: entry.proxy })
        .filter(([, v]) => v !== '' && v !== undefined),
    )
    if (Object.keys(defaults).length) await writeConfigFile(defaults)
  }
  await rebuild()
  ctx.on('dispose', () => stop())
}
