...
export async function handleCommand({ profile, action, spec, proxy }) {
  ...
  return viaNode
    ? run(process.execPath, [bin, ...dshArgs], { timeout: 600000, env })
    : run(bin, dshArgs, { timeout: 600000, env, shell: process.platform === 'win32' })
}

function run(bin, args, { timeout, shell, env } = {}) {
